...
Eigen::Matrix3d eulerRates2bodyRates(Eigen::Vector3d eulerAngles)
{
    ...
    Eigen::Matrix3d R;//惯性系下的欧拉角速度转到body系
    R<<  1,   0,    -sp,
            0,   cr,   sr*cp,
            0,   -sr,  cr*cp;
...
void IMU::addIMUnoise(MotionData& data)//传进一个结构体
{
    std::random_device rd;//随机数
    std::default_random_engine generator_(rd());//随机数
    std::normal_distribution<double> noise(0.0, 1.0);//正太高斯分布

    Eigen::Vector3d noise_gyro(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d gyro_sqrt_cov = param_.gyro_noise_sigma * Eigen::Matrix3d::Identity();//协方差
    //陀螺仪的误差模型
