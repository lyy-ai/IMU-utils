...
void save_points(std::string filename,std::vector<Eigen::Vector4d,Eigen::
aligned_allocator<Eigen::Vector4d>> points)
{
    ...
    for(int i=0;i<points.size();++i)
    {
        ...
        save_points<<p(0)<<" "<<p(1)<<" "<<p(2)<<" "<<p(3)<<std::endl;
    }
}
...
void save_Pose(std::string filename,std::vector<MotionData> pose)
{
    ...
    {
        MotionData data=pose[i];//传进来一个结构体
        ...
        save_points<<time<<" "<<q.w()<<" "<<q.x()<<" "<<q.y()<<" "<<q.z()<<" "
        <<t(0)<<" "<<t(1)<<" "<<t(2)<<" "<<gyro(0)<<" "<<gyro(1)<<" "<<gyro(2)<<" "
...
void LoadPose(std::string filename,std::vector<MotionData>& pose)
{
    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            ss>>time;
            ss>>q.w();ss>>q.x();ss>>q.y();ss>>q.z();ss>>t(0);ss>>t(1);ss>>t(2);
            ss>>gyro(0);ss>>gyro(1);ss>>gyro(2);ss>>acc(0);ss>>acc(1);ss>>acc(2);
